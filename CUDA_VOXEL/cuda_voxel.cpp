...
const char* OutputFormats[] = { "binvox file", "morton encoded blob", "obj file (pointcloud)", "obj file (cubes)", "magicavoxel file" };
...
// 重新写一个体素化函数，修改输出输出的格式，可以被直接调用，不需要再调用main函数
...
void cuda_voxelizeMesh(
	...
	mesh->need_faces(); // Trimesh: Unpack (possible) triangle strips so we have faces for sure
	fprintf(stdout, "[Mesh] Number of triangles: %zu \n", mesh->faces.size());
	fprintf(stdout, "[Mesh] Number of vertices: %zu \n", mesh->vertices.size());
	fprintf(stdout, "[Mesh] Computing bbox \n");
	mesh->need_bbox(); // Trimesh: Compute the bounding box (in model coordinates)

	// COMPUTE BOUNDING BOX AND VOXELISATION PARAMETERS
	fprintf(stdout, "\n## VOXELISATION SETUP \n");

	// Initialize our own AABox, pad it to the same size as the container
	float3 bbox_min, bbox_max;
	// The bbox_max is equal to the bbox_min of the mesh added to the voxel_size multiplied by the grid size
	bbox_max.x = mesh->bbox.min.x + voxel_size * gridX;
	bbox_max.y = mesh->bbox.min.y + voxel_size * gridY;
	bbox_max.z = mesh->bbox.min.z + voxel_size * gridZ;
	bbox_min.x = mesh->bbox.min.x;
	bbox_min.y = mesh->bbox.min.y;
	bbox_min.z = mesh->bbox.min.z;

	AABox<float3> bbox_mesh_container;
	bbox_mesh_container.min = bbox_min;
	bbox_mesh_container.max = bbox_max;

	// Create voxinfo struct and print all info
	voxinfo voxelization_info(bbox_mesh_container, make_uint3(gridX, gridY, gridZ), mesh->faces.size());
	...
	unsigned int* vtable_surface = 0; // Both voxelization paths (GPU and CPU) need this
	...
	if (cuda_ok && !forceCPU) {
		...
		device_triangles = meshToGPU_managed(mesh);
		...
		checkCudaErrors(cudaMallocManaged((void**)&vtable_surface, vtable_size));


		fprintf(stdout, "\n## GPU VOXELISATION \n");
		if (solidVoxelization) {
			voxelize(voxelization_info, device_triangles, vtable_surface, (outputformat == OutputFormat::output_morton));
		...
		if (!solidVoxelization) {
			cpu_voxelizer::cpu_voxelize_mesh(voxelization_info, mesh, vtable, (outputformat == OutputFormat::output_morton));
		}
		else {
			cpu_voxelizer::cpu_voxelize_mesh_solid(voxelization_info, mesh, vtable, (outputformat == OutputFormat::output_morton));
		}
	}
	...
	for(int x = 0; x < gridX; x++)
		for(int y = 0; y < gridY; y++)
			for (int z = 0; z < gridZ; z++)
			{
				...
				if(checkVoxel(x, y, z,voxelization_info.gridsize, vtable) || checkVoxel(x, y, z, voxelization_info.gridsize, vtable_surface))
					voxelGrid[idx] = true;
				else
					voxelGrid[idx] = false;
			}

	checkCudaErrors(cudaFree(vtable));
	checkCudaErrors(cudaFree(vtable_surface));
...
int CommandLineFunction(int argc, char* argv[]) {
	// add argv
	string str = "cuda_voxelizer -f F:/Project/cuda_voxelizer/test_models/Object_0.off -s 32 -o obj -solid";
	argv = (char**)malloc(8 * sizeof(char*));
	int itr = 0;
	for (int i = 0; i < 8; i++) {
		char* p = (char*)malloc(100 * sizeof(char));
		for (int j = 0; j < 100 && itr < str.length() && str[itr] != ' '; j++, itr++) {
			p[j] = str[itr];
			p[j + 1] = 0;
		}
		itr++;
